...
class scanner:
    ...
    def preprocessing(self):
        ...
        imgsh=cv2.resize(self.img,(700,700));
        cv2.imshow("board",imgsh )
        cv2.waitKey(0) # waits until a key is pressed
        cv2.destroyAllWindows() # destroys the window showing image

        img_blur = cv2.GaussianBlur(self.img, (3, 3), 0)
        img_blur_sh=cv2.resize(img_blur,(600,600))
        ...
        canny_output_sh=cv2.resize(canny_output,(600,600))
        ...
        '''
        ######
        cv2.imshow("blurred",img_blur_sh)
        cv2.waitKey(0) # waits until a key is pressed
        cv2.destroyAllWindows() # destroys the window showing image
        #######
        ######  
        cv2.imshow("canny",canny_output_sh)
        cv2.waitKey(0) # waits until a key is pressed
        cv2.destroyAllWindows() # destroys the window showing image
        #######
        ######
        binary_output_sh=cv2.resize(binary_output,(600,600))
        cv2.imshow("binary ",binary_output_sh)
        cv2.waitKey(0) # waits until a key is pressed
        cv2.destroyAllWindows() # destroys the window showing image
        #######

        cv2.imshow("final ",square)
        cv2.waitKey(0) # waits until a key is pressed
        cv2.destroyAllWindows() # destroys the window showing image
        '''
    ...
    def getDigits(self):
        ...
        for x in range(9):
            for y in range(9):
                ...
                s = int(900*0.13) # 117 1/9=0.11 + extra 0.02 for line width
                elm = np.zeros((s, s))
                #elm is a 2d array of images
                #populating elm with required images
                for i in range(s):
                    for j in range(s):
...
"""
s = scanner("/home/zoh/Programming_Data/Projects/Sudoku_Solver/Pics/Train_data/4.jpg")
